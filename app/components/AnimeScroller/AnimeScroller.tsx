...
const AnimeScroller = ({ title, items }: AnimeScrollerProps) => {
              ...
              <div className="relative aspect-[2/3]">
                ...
                <div className="absolute bottom-4 left-4 right-4 p-2 bg-black/50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="flex items-center gap-1">
                    <span className="text-sm font-semibold text-white">Score: {anime.score}</span>
                  </div>
                  <p className="text-sm text-white line-clamp-2">{anime.type}</p>
                </div>
              </div>
              
              <div className="p-4">
                <h3 className="font-semibold text-white line-clamp-2">{anime.title}</h3>
              </div>
            </Card>
            </Link>
          </motion.div>
